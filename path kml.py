# -*- coding: utf-8 -*-
"""
Created on Wed Aug 20 16:59:13 2025

@author: hp
"""

"""
Drone Flight Path Generator for Mission Planner

This script processes KML/KMZ polygon files to generate a lawnmower-style flight path
and exports waypoints in a format compatible with Mission Planner.

Key Features:
- Extracts polygon coordinates from KML/KMZ files.
- Projects coordinates to UTM for accurate distance calculations.
- Generates an optimized lawnmower (back-and-forth) coverage path within the polygon.
- Saves waypoints with commands for:
    * Home (dummy) position (Index 0)
    * Takeoff (Index 1)
    * Speed change (Index 2)
    * Navigation waypoints (Index 3+)
    * Return to Launch (last waypoint)
- Outputs a waypoint file that Mission Planner can import directly.

Usage:
- Set the input KML/KMZ file path.
- Configure altitude (meters), line spacing (meters), and drone speed (m/s).
- Run the script to generate waypoints.
- Import the generated `.waypoints` file into Mission Planner for mission execution.

Notes:
- The output folder path can be changed by modifying the OUTPUT_FOLDER variable.
- Ensure the polygon in the KML/KMZ file is closed (first and last point the same).
- Waypoints include necessary MAVLink commands for proper mission flow.
- Tested with Mission Planner, but format may also work with QGroundControl.

Dependencies:
- shapely, geopandas, pyproj, lxml or xml.etree.ElementTree
